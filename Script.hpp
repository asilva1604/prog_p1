...
namespace prog
{
  ...
  {
  public: 
    Script(const std::string &filename);  //standard constructor
    ~Script();                            //destructor
    void run();                           //function that runs script
  private:
    // Current image.
    Image *image;
  ...
  private:
    // Private functions
    void clear_image_if_any();            //deletes image, if any open
    void open();                          //open image
    void blank();                         //makes a blank image
    void save();                          //saves image
    ...
    void xpm2_open();                     //open image in the XPM2 format
    void xpm2_save();                     //save image in the XPM2 format
